#!/usr/bin/env python3
"""
Script para extraer preguntas de DevOps del archivo PDF de 380 preguntas
Basado en el libro "380+ DevOps Interview Questions and Answers" de Salunke, Manish
"""
...
class DevOps380Extractor:
    def __init__(self, pdf_path):
        """
        Inicializa el extractor con la ruta del archivo PDF
        
        Args:
            pdf_path (str): Ruta al archivo PDF de DevOps (380 preguntas)
        """
    ...
    def extract_questions(self):
        ...
        # Patrones para identificar preguntas (adaptados para 380 preguntas de DevOps)
        question_patterns = [
    ...
    def categorize_questions(self):
        """
        Categoriza las preguntas por temas de DevOps
        """
        ...
        keywords = {
            'docker': ['docker', 'container', 'image', 'dockerfile', 'docker-compose', 'registry'],
            'kubernetes': ['kubernetes', 'k8s', 'pod', 'deployment', 'service', 'namespace', 'helm'],
            'cicd': ['ci/cd', 'continuous integration', 'continuous deployment', 'pipeline', 'jenkins', 'gitlab', 'github actions'],
            'terraform': ['terraform', 'infrastructure as code', 'iac', 'provider', 'resource', 'state'],
            'ansible': ['ansible', 'playbook', 'inventory', 'role', 'module', 'ad-hoc'],
            'monitoring': ['monitoring', 'prometheus', 'grafana', 'elk', 'logging', 'alerting', 'metrics'],
            'jenkins': ['jenkins', 'pipeline', 'groovy', 'job', 'build', 'deploy'],
            'git': ['git', 'version control', 'repository', 'branch', 'merge', 'commit'],
            'linux': ['linux', 'bash', 'shell', 'systemd', 'process', 'file system'],
            'networking': ['network', 'dns', 'load balancer', 'proxy', 'firewall', 'vpc'],
            'security': ['security', 'rbac', 'secrets', 'authentication', 'authorization', 'ssl'],
    ...
    def run_extraction(self, raw_output="devops_380_questions_raw.txt", 
                      structured_output="devops_380_questions_structured.json"):
        """
        ...
        """
        print("🚀 Iniciando extracción de preguntas de DevOps (380)")
        print("=" * 60)
...
def main():
    ...
    """
    # Configuración
    pdf_file = "380_ DevOps Interview Questions and Answer - Salunke, Manish.pdf"
    raw_output = "devops_380_questions_raw.txt"
    structured_output = "devops_380_questions_structured.json"
    ...
    extractor = DevOps380Extractor(pdf_file)
