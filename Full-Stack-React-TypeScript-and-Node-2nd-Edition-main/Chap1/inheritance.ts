
// Chap1/inheritance.ts
// This code defines a class hierarchy for items, specifically bicycles and scooters.
// It includes a factory for creating items based on data input.
// The code is designed to be extensible, allowing for easy addition of new item types and creation logic.
// It uses TypeScript's class system to ensure type safety and encapsulation.

class Item {
  id: string;
  description: string;
  price: number;

  getId(): string {
    return this.id;
  }
}

class Bicycle extends Item {
  wheelCount: number;

  getWheelCount(): number {
    return this.wheelCount;
  }
}

const bicycle = new Bicycle();
bicycle.id = "123";
bicycle.description = "Mountain Bike";
bicycle.price = 299.99;
bicycle.wheelCount = 2;
console.log("id", bicycle.getId());
console.log("wheel count", bicycle.getWheelCount());
